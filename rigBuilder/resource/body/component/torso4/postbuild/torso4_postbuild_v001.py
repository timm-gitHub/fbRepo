...
class RigBuild(classes.RigComponent):
    ...
    def build(self):
        ...
        cmds.setAttr('%s.overrideColor' % shp,env.CONTROL_COLOR['C'])
        ...
        cmds.connectAttr('%s_spine2Ik_ctl_%s.sx' % (self.locus,self.count),'%s_scaleBlendback_bcl_%s.color1B' % (self.locus,self.count))
        cmds.connectAttr('%s_spine2Ik_ctl_%s.sy' % (self.locus,self.count),'%s_scaleBlendback_bcl_%s.color1R' % (self.locus,self.count))
        cmds.connectAttr('%s_spine2Ik_ctl_%s.sz' % (self.locus,self.count),'%s_scaleBlendback_bcl_%s.color1G' % (self.locus,self.count))
        ...
        posA = cmds.xform('%s_back_bnd_%s' % (self.locus,self.count),q=True,ws=True,t=True)
        ...
        back    = cmds.circle(ch=False)[0]
        chest   = cmds.circle(ch=False)[0]
        neck    = cmds.circle(ch=False)[0]
        new     = [neck,chest,back]
        old     = ['%s_neckFk_ctl_%s' % (self.locus,self.count),'%s_chestFk_ctl_%s' % (self.locus,self.count),'%s_backFk_ctl_%s' % (self.locus,self.count)]
...
#         if cmds.objExists('%s_headFk_ctj_%s_parentConstraint1' % (self.locus,self.count)):
#             cmds.delete('%s_headFk_ctj_%s_parentConstraint1' % (self.locus,self.count))
#         cmds.setAttr('%s_neckFk_ctj_%s.rz' % (self.locus,self.count),0)
#         cmds.parentConstraint('%s_neck_bnd_%s' % (self.locus,self.count),'%s_headFk_ctj_%s' % (self.locus,self.count),mo=True)
        ...
        for attr in ['sx','sy','sz']:
            cmds.setAttr('%s_backFk_ctl_%s.%s' % (self.locus,self.count,attr),l=True,k=False)
